%final prokect OLG_IDIO_AGG_RISK

function OLG_IDIO_AGG_RISK
...
% guess psi0 and psi1 
psi0=[5.0 ;9.0];            %maybe kann ich das noch soft coden (initially hatte ich [1 0.9] und [alpha alpha]
psi1=[0.030; -0.80];
% SOLUTION

func_par(opt_det,opt_nosr,opt_ny);
...
psi=[psi0; psi1];
display(psi)
%  save vrep0final.txt VV -ascii -double 
%  save gridx0final.txt gridxx -ascii -double
%  save vrep0finalcon.txt VVcon -ascii -double 
%  save gridx0finalcon.txt gridxcon -ascii -double
end %main function 

function func_par(opt_det,opt_nosr,opt_ny)
...
%simulation of shocks for eta and zeta (both generated by independent
%markov chains)
nshocks=100;
zeta0=1;
% mcz = dtmc(piz);
...
% ETA=zeros(nshocks,nj);
% for ss=1:nj
%     ETA(:,ss) = simulate(mce,nshocks,'X0',eta0(ss));
% end
...
kproj3=8;%check these values  %equilibrium aggregate capital taken from project 3 
